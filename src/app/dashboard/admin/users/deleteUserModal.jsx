...
const DeleteUserModal = ({
  ...
  const handleDeleteUser = async () => {
    ...
    try {
      await axios.delete(
        `http://iro-website-bn-vx04.onrender.com/api/Inventory/users/delete-user/${userIdToDelete}`,
      );
      ...
      >
        ...
        <div className="flex flex-col">
          <p id="delete-user-description" className="text-center">
            ...
            {userIdToDelete
              ? users.find((user) => user.id === userIdToDelete)?.id
              : "this user"}
            ?
          </p>
          <div
            className="flex items-start justify-start mt-4 text-red-500 py-2
                        bg-gradient-to-r from-red-500 from-1% via-red-500 via-10% to-[#D9D9D9] to-10%"
          >
            <IoWarning className="ml-10 text-[24px]" />
            <p className="text-black flex flex-col items-start text-md pl-2 font-bold">
              Warning:{" "}
              <span className="font-normal text-red-500 text-sm">
                By deleting this user, you won&apost access them again.
              </span>
            </p>
          </div>
        </div>
        <DialogFooter className="flex justify-between mt-6">
          ...
          <Button
            onClick={onClose}
            className="bg-gray-300 text-gray-700 px-8 py-2"
            disabled={isLoading}
          >
            No, cancel
          </Button>
